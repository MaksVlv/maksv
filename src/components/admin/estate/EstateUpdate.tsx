...
export default function EstateUpdate({
  estateOld,
  onCloseClick,
  onUpdate,
  googleApi,
  ...
  useEffect(() => {
    ...
    axios.get('city?size=10000&sort=name.lv:asc').then(
      (res) => {
  ...
  const handleNameSubmit = (e: React.SyntheticEvent, update: string) => {
    ...
    formData.append(
      'estate',
      JSON.stringify({ ...estate, images: [], mainImage: '' }),
    );
    axios
      .post(`estate/update?update=${update}`, formData, {
        headers: {
  ...
  const getDistricts = (cityId: string) => {
    setEstate({
      ...estate,
      city: { ...estate.city, _id: cityId },
      district: { name: { lv: '', ru: '', en: '' }, _id: '' },
    });
    axios.get(`city/district?city=${cityId}&sort=name.lv:asc`).then(
      (res) => {
  ...
  const mainImageChange = (file: Image) => {
    if (!file) return;
    if (file && file.preview === estate.mainImage) return;
    setLoading(true);
    ...
    axios
      .post(`estate/update?update=mainImage`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
  ...
  const imagesChange = async (files: Image[], length: number | undefined) => {
    if (typeof length === 'undefined') {
      ...
      if (changedFilesPreview.join() !== estate.images.join()) {
        ...
        formDataImages.append(
          'estate',
          JSON.stringify({ ...estate, images: changedFilesPreview }),
        );
        await axios
          .post('estate/update?update=imageChangeOrder', formDataImages, {
            headers: {
              'Content-Type': 'multipart/form-data',
              Authorization: `Bearer ${localStorage.getItem('token')}`,
            },
    ...
    for (let i = 0; i < imagesSpliced.length; i++) {
      ...
      await axios
        .post('estate/update?update=image', formDataImages, {
          headers: {
            'Content-Type': 'multipart/form-data',
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
        ...
        .catch((err) => {
          ...
          toast.success(
            'Not all images are uploaded, please check estate later',
          );
        });
    }

    await axios
      .get(`estate/info?id=${estateOld._id}&disabled=true`)
      .then((res) => {
        // @ts-ignore
        setEstate({ ...estate, images: res.data.images });
      })
  ...
  const videoChange = (video: File | null) => {
    ...
    formData.append(
      'estate',
      JSON.stringify({ ...estate, images: [], mainImage: '', video: '' }),
    );
    if (video) {
      formData.append('video', video);
    }
    axios
      .post(`estate/update?update=video`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
  ...
  const imagesDelete = (files: Image[], url: string) => {
    ...
    axios
      .post(`estate/update?update=imageDelete`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
  ...
  const changeDraft = (status: boolean) => {
    ...
    axios
      .post(`estate/update?update=disabled`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
  ...
  const changeType = (e: ChangeEvent<HTMLSelectElement>) => {
    ...
    setEstate({
      ...
      // @ts-ignore
      type: {
        lv: types[typeIndex].lv,
        ru: types[typeIndex].ru,
        en: types[typeIndex].en,
      },
      rooms: '',
      floor: '',
      livingArea: '',
  ...
  const deleteAllImages = async () => {
    ...
    for (let i = 0; i < estate.images.length; i++) {
      formData.append('url', estate.images[i]);
      await axios.post(`estate/update?update=imageDelete`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
    ...
    toast.success(`Estate images deleted`);
    // @ts-ignore
    setEstate({ ...estate, images: [] });
    setLoading(false);
  };
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 flex items-center justify-center z-10">
        <div
          className="bg-white shadow-lg p-8 w-full relative"
          style={{ maxHeight: '90vh', maxWidth: '800px', overflow: 'auto' }}
        >
          <svg
          ...
          <div className="mb-6">
                ...
                <div className={'flex-1'}>
                  <input
                    ...
                    onChange={(e) =>
                      setEstate({
                        ...estate,
                        name: { ...estate.name, lv: e.target.value },
                      })
                    }
                  />
                </div>
                <div className={'flex-1'}>
                  <input
                    ...
                    onChange={(e) =>
                      setEstate({
                        ...estate,
                        name: { ...estate.name, ru: e.target.value },
                      })
                    }
                  />
                </div>
                <div className={'flex-1'}>
                  <input
                    ...
                    onChange={(e) =>
                      setEstate({
                        ...estate,
                        name: { ...estate.name, en: e.target.value },
                      })
                    }
                  />
                </div>
            ...
            <form
              onSubmit={(e: React.SyntheticEvent) =>
                handleNameSubmit(e, 'description')
              }
            >
            ...
            >
              ...
              <div className="mb-4 flex justify-between gap-3">
                <input
                  ...
                  value={estate.price}
                  onChange={(e) =>
                    setEstate({ ...estate, price: Number(e.target.value) })
                  }
                />
                <div className="flex justify-end relative">
                  <button
            ...
            >
              ...
              <div className="mb-4 flex justify-between gap-3">
                <label className={styles.switch}>
                  <input
                    type="checkbox"
                    defaultChecked={estate.rent}
                    onChange={() =>
                      setEstate({ ...estate, rent: !estate.rent })
                    }
                  />
            ...
            <form
              onSubmit={(e: React.SyntheticEvent) =>
                handleNameSubmit(e, 'location')
              }
            >
              <div className="mt-4 block text-gray-700 font-bold mb-2">
                Pilsēta/novads:
              </div>
              <div className="relative inline-block w-full">
                ...
                >
                  <option value="" disabled>
                    Izvēlieties objekta pilsēta/novads
                  </option>
                  {cities.map((city, i) => (
                    <option value={city._id} key={i + city._id}>
                      {city.name.lv}
                    </option>
                  ))}
                </select>
                <div className="absolute inset-y-0 right-0 flex items-center px-2 pointer-events-none">
                  <svg className="w-4 h-4 fill-current" viewBox="0 0 20 20">
                    <path d="M14.14 7.88l-4.95 4.95a1.5 1.5 0 01-2.12 0l-4.95-4.95a1.5 1.5 0 012.12-2.12L10 9.77l3.05-3.05a1.5 1.5 0 012.12 2.12z" />
                  </svg>
                </div>
              </div>
              ...
              <div className="relative inline-block w-full mb-4">
                <select
                  ...
                  onChange={(e) =>
                    setEstate({
                      ...estate,
                      district: { ...estate.district, _id: e.target.value },
                    })
                  }
                >
                  <option value="" disabled>
                    Izvēlieties objekta pagasts/mikrorajons
                  </option>
                  {districts.map((district, i) => (
                    <option value={district._id} key={i}>
                      {district.name.lv}
                    </option>
                  ))}
                </select>
            ...
            <Upload
              one={true}
              onFileChange={(file: Image[]) => mainImageChange(file[0])}
              filesOld={[estate.mainImage]}
              deleteImg={false}
              loading={(state: boolean) => setLoading(state)}
            />

            <div className={'flex justify-end bg-red-400 py-2 mt-6'}>
              <button
                type={'button'}
                className="bg-red-600 hover:bg-red-900 text-white py-2 px-4 disabled:cursor-not-allowed"
            ...
            <div className="block text-gray-700 font-bold">Images:</div>
            <Upload
              onFileChange={(files: Image[], length: number | undefined) =>
                imagesChange(files, length)
              }
              filesOld={estate.images}
              onDeleteImg={(files: Image[], url: string) =>
                imagesDelete(files, url)
              }
              loading={(state: boolean) => setLoading(state)}
            />
            ...
            <form
              onSubmit={(e: React.SyntheticEvent) =>
                handleNameSubmit(e, 'youtube_link')
              }
            >
              <div className="block text-gray-700 font-bold mt-6">
                Youtube video:
              ...
              <input
                ...
                onChange={(e) =>
                  setEstate({ ...estate, youtube_link: e.target.value })
                }
              />
              <div className="flex justify-end relative">
                <button
            ...
            <VideoInput
              file={estate.video}
              onChange={(video: File | null) => videoChange(video)}
            />
            ...
            <div className="relative inline-block w-full mb-4">
              <select
                ...
                defaultValue={Object.keys(types).find(
                  (key) => types[key].lv === estate.type.lv,
                )}
              >
                <option value="" disabled>
                  Select estate type
            ...
            {estate.type.lv === 'Māja' && (
              <form
                onSubmit={(e: React.SyntheticEvent) =>
                  handleNameSubmit(e, 'house')
                }
              >
                <div className={'flex flex-col'}>
                  <div className="block text-gray-700 font-bold mb-4 text-center">
                    HOUSE
                  </div>
                  <div className="block text-gray-700 font-bold mb-2">
                    Rooms:
                  ...
                  <input
                    ...
                    value={estate.floor}
                    onChange={(e) =>
                      setEstate({ ...estate, floor: e.target.value })
                    }
                  />
                  <div className="block text-gray-700 font-bold mb-2">
                    Living area:
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEstate({ ...estate, livingArea: e.target.value })
                    }
                  />
                  <div className="block text-gray-700 font-bold mb-2">
                    Land area:
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEstate({ ...estate, landArea: e.target.value })
                    }
                  />
                  <div className="block text-gray-700 font-bold mb-2">
                    Cadastral number:
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEstate({ ...estate, cadastralNumber: e.target.value })
                    }
                  />
                </div>
                <div className="flex justify-end relative">
                ...
                <div className={'flex flex-col'}>
                  ...
                  <div className="relative inline-block w-full mb-4">
                    <select
                      ...
                      defaultValue={
                        Object.keys(series).findIndex(
                          (key) => series[key].en === estate.series?.en,
                        ) + 1
                      }
                    >
                      <option value="" disabled>
            ...
            {(estate.type.lv === 'Rūpnīca' ||
              estate.type.lv === 'Zeme' ||
              estate.type.lv === 'Komerciālais īpašums') && (
              <form
                onSubmit={(e: React.SyntheticEvent) =>
                  handleNameSubmit(e, 'land')
                }
              >
                <div className={'flex flex-col'}>
                  <div
                    className="block text-gray-700 font-bold mb-4 text-center"
                    ...
                    <div className={'mb-4'}>
                      ...
                      <div className="relative inline-block w-full mb-4">
                        <select
                          ...
                          defaultValue={
                            Object.keys(assignment).findIndex(
                              (key) =>
                                assignment[key].en === estate.assignment?.en,
                            ) + 1
                          }
                        >
                          <option value="" disabled>
                            Select land assignment
                          </option>
                          {Object.keys(assignment).map(
                            (key: string, i: number) => (
                              <option value={key} key={i}>
                                {assignment[key].lv}
                              </option>
              ...
              >
                <div className={'flex flex-col'}>
                  ...
                  <input
                    ...
                    value={estate.size}
                    onChange={(e) =>
                      setEstate({ ...estate, size: e.target.value })
                    }
                  />
                  <div className="block text-gray-700 font-bold mb-2">
                    Gate height:
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEstate({
                        ...estate,
                        gateHeight: Number(e.target.value),
                      })
                    }
                  />
                </div>
