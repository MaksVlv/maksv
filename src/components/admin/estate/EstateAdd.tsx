...
export default function EstateAdd({
  ...
  const handleSubmit = (
    ...
    axios
      .post(`estate/add?disabled=${disabled}`, formData, {
        headers: {
      ...
      .then(
        async (res) => {
          ...
          for (let k = 0; k < imagesSpliced.length; k++) {
            for (let i = 0; i < imagesSpliced[k].length; i++) {
              ...
              formDataImages.append(
                'estate',
                JSON.stringify({ ...res.data, images: [], mainImage: '' }),
              );
          ...
          if (estateVideo) {
            ...
            await axios
              .post(`estate/update?update=video`, formDataWithVideo, {
                headers: {
                  'Content-Type': 'multipart/form-data',
                  Authorization: `Bearer ${localStorage.getItem('token')}`,
                },
              ...
              .catch(() => {
                toast.warn(
                  'Error with uploading video, please check estate later',
                );
              });
          }
  ...
  const getCombinedAddress = () => {
    return [
      estate.street,
      cities.find((city) => city._id === estate.city)?.name?.lv,
      districts.find((district) => district._id === estate.district)?.name?.lv,
    ]
      .filter(Boolean)
      .join(', ');
  };
        ...
        >
          {loading && (
            <>
              ...
              />
              <div
                className={'justify-center items-center fixed'}
                style={{
                  zIndex: 500,
                  left: '50%',
                  top: '50%',
            ...
            <form onSubmit={handleSubmit} ref={formRef}>
              ...
              <div className="relative inline-block w-full mb-4">
                <select
                  ...
                  onChange={(e) =>
                    setEstate({ ...estate, district: e.target.value })
                  }
              ...
              <Upload
                ...
                onFileChange={(file: Image[]) =>
                  setEstate({ ...estate, mainImage: file[0] })
                }
              ...
              <Upload
                onFileChange={(files: Image[]) => imagesChange(files)}
                loading={(state: boolean) => setLoading(state)}
              />
              ...
              <VideoInput
                onChange={(video: File | null) => setEstateVideo(video)}
              />
