...
export default function Estates({ googleApi }: { googleApi: string }) {
  ...
  const [isRent, setIsRent] = useState<string>('null');
  ...
  useEffect(() => {
    getData(pagination, search, sort, isRent);
  ...
  const searchChange = (e: any) => {
    ...
    timeoutId.current = setTimeout(() => {
      getData({ ...pagination, page: 0 }, e.target.value, sort, isRent, true);
    }, 700);
  };
  ...
  const getData = (
    ...
    axios
      .get(
        `estate?size=${pagination.size}&page=${pagination.page}&search=${search}&disabled=true&sort=${sort}${isRent !== 'null' ? `&rent=${isRent}` : ''}`,
      )
      .then(
        (res) => {
  ...
  const changeSort = (val: string) => {
    setSort(val);
    getData({ ...pagination, page: 0 }, search, val, isRent, true);
  };

  const changeIsRent = (val: string) => {
    setIsRent(val);
    getData({ ...pagination, page: 0 }, search, val, val, true);
  };

  return (
    <section className="bg-white w-full container mx-auto px-8 mt-10">
      <h1 className="w-full text-center text-2xl font-bold pt-8 pb-3">
        Estates
      </h1>
      <div className="relative mb-6 z-0 flex">
        ...
        <div>
          <select
            ...
            id="is_rent"
            value={isRent}
            onChange={(e) => changeIsRent(e.target.value)}
          >
          ...
          {estates.length > 0 ? (
            estates.map((estate, i) => (
              <div
                ...
                style={{
                  backgroundColor: `${estate.disabled ? '#FFCDD2' : ''}`,
                }}
                key={i}
                onClick={() => {
                  setIsOpenUp(i.toString());
                }}
              >
                <div
                  className={'text-[#1E1E1E] font-bold text-1xl ' + styles.info}
                  style={{ width: '5%' }}
                >
                  <img
                    src={estate.mainImage}
                    alt={estate.name.lv + 'MainImage'}
                  />
                </div>
                <div
                  className={'text-[#1E1E1E] font-bold text-1xl ' + styles.info}
                ...
                <div
                  ...
                  style={{ width: '5%' }}
                >
                  {estate.rent ? 'RENT' : 'SELL'}
                </div>
                <div
                  className={'text-[#1E1E1E] font-bold text-1xl ' + styles.info}
                  style={{ width: '8%' }}
                >
                  {Number(estate.price.toFixed(2)).toLocaleString('en-US', {
                    style: 'currency',
                    currency: 'EUR',
                  })}
                </div>
      ...
      {isOpenAdd && (
        <EstateAdd
          googleApi={googleApi}
          onCloseClick={() => setIsOpenAdd(false)}
          onSave={() => {
      ...
      {isOpenUp && (
        <EstateUpdate
          googleApi={googleApi}
          estateOld={estates[Number(isOpenUp)]}
          onCloseClick={() => setIsOpenUp('')}
          onUpdate={() => {
            setForceUpdate(forceUpdate + 1);
          }}
        />
