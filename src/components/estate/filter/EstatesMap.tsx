...
export const EstatesMap = ({ googleApi, fullLine }: Props) => {
    ...
    const loadObjects = (map: google.maps.Map) => {
        ...
        axios.get(`estate?search=${fil.search}&rent=${fil.rent}&type=${fil.type}&city=${fil.city}&district=${fil.district}&priceFrom=${fil.priceFrom}&priceTill=${fil.priceTill}&floorFrom=${fil.floorFrom}&floorTill=${fil.floorTill}&roomsFrom=${fil.roomsFrom}&roomsTill=${fil.roomsTill}&livingAreaFrom=${fil.livingAreaFrom}&livingAreaTill=${fil.livingAreaTill}&landAreaFrom=${fil.landAreaFrom}&landAreaTill=${fil.landAreaTill}&gateHeightFrom=${fil.gateHeightFrom}&gateHeightTill=${fil.gateHeightTill}&series=${fil.series}&assignment=${fil.assignment}&map=1`).then(res => {
            ...
            if (res.data.data.length > 0) {
                ...
                map.setCenter({ lat: 56.874310, lng: 24.378061 })
                map.setZoom(5);
            }
...
const getEstatesCenter = (estates: IEstate[]): {center: {lat: number, lng: number}, zoom: number} => {
    const tmp: any = [];
